...
client.on('messageCreate', async message => {
    ...
    if (isAdmin) {
        if (message.content.startsWith('!open')) {
            ...
            hypeInterval = setInterval(() => {
                if (gatesOpen) message.channel.send(messages.getRandom(messages.hypeQuotes));
            }, 48 * 60 * 60 * 1000); // 48 Hour Reminder
        }
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'signup_modal') {
        ...
        const row = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId('boss_select').setPlaceholder('Which bosses?')
                .addComponents(
                    { label: 'All Bosses', value: 'All' },
                    { label: 'Gold Bosses Only', value: 'Gold' },
                    { label: 'Specific Request', value: 'Specific' }
                )
        );

        db.prepare('INSERT INTO signups (discord_user_id, character_name, level, status) VALUES (?, ?, ?, ?)')
          .run(interaction.user.id, charName, level, 'PENDING_BOSS');

        await interaction.reply({ content: `Next: Select boss preference for **${charName}**:`, components: [row], flags: MessageFlags.Ephemeral });
    }

    if (interaction.isStringSelectMenu() && interaction.customId === 'boss_select') {
        const choice = interaction.values[0];
        db.prepare('UPDATE signups SET boss_choice = ?, status = "PENDING_VOC" WHERE discord_user_id = ? AND status = "PENDING_BOSS"')
          .run(choice, interaction.user.id);

        const row = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId('voc_select').setPlaceholder('Your Vocation...')
                .addComponents(
        ...
        await interaction.update({ content: `Last step: Vocation for this raid:`, components: [row], flags: MessageFlags.Ephemeral });
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'voc_select') {
        ...
        await interaction.update({ content: `✅ Registered! Your loyalty is noted. Use !roster in the raid channel to see the list.`, components: [], flags: MessageFlags.Ephemeral });
    }
});

// Standard displayRoster, updateOnlineTracker, runTracker, and helper functions follow...
