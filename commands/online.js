...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('online')
        .setDescription('Pulls the online list for tracked guilds and characters.'),
    adminOnly: false,
    async execute(interaction, client, db) {
        // We defer so Discord doesn't timeout while we fetch from TibiaData
        ...
        try {
            ...
            if (output.length === 0) {
                return await interaction.editReply('💨 **The lands are quiet.** None of our tracked targets are currently online.');
            }
            ...
            await interaction.editReply('⚠️ **The scouts were ambushed!** Failed to connect to the TibiaData API.');
        }
    },
};
